...
Stringをコードポイント配列に変換する際にStringで直接扱うよりChar配列に変換して扱う方が高速化するように（<a href="https://www.ibm.com/developerworks/jp/java/library/j-unicode/" target="_blank">Java 言語による Unicode サロゲート・プログラミング</a>）、低レイヤで処理を書けば高速化したり（メモリ消費を減らしたり）できるのは試す前に想定できることです（その分、プログラムが長大化し読みづらくなる、変数が増えてバグが発生する可能性が増えるといった短所もあります）が、実際に確認する作業は大切なことです。<br>
