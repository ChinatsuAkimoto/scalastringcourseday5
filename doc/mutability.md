...
Stringをコードポイントに変換する際にStringのcodePointAt/codePointBeforeメソッドよりCharacterクラスのcodePointAt/codePointBeforeメソッドの方が、Stringで直接扱うよりChar配列に変換して扱う方が、より高速化するように（<a href="https://www.ibm.com/developerworks/jp/java/library/j-unicode/" target="_blank">Java 言語による Unicode サロゲート・プログラミング</a>）、lower levelで処理を書けば高速化したり（メモリ消費を減らしたり）できることは<a href="https://github.com/ynupc/scalastringcourseday3/blob/master/doc/implementation.md" target="_blank">Day 3</a>で既に学習しましたので、我々はjoinの処理速度についても試す前に想定できるはずです（lower levelになる分、プログラムが長大化し読みづらくなる、変数が増えてバグが発生する可能性が増えるといった短所もあります）。しかし、想定通りの挙動であるかを実際に確認する作業は大切なことです。<br>
